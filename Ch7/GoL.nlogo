...
to setup
  clear-all
  ask patches [
    if random 100 < initial-density [ set pcolor white ]
...
to go
  ask patches [
    let alife count neighbors with [pcolor = white]
    ifelse pcolor = black
    [ if alife = 3 [set next-color white]]
    [ if alife = 2 or alife = 3 [set next-color white]]
  ]
  ask patches [
